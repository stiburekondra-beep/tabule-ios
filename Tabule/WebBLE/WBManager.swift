...
//
//  Upraveno pro Tabuli na zápěstí (Baklažán, 22. 9. 2026): appka je
//  SwiftUI, ne storyboard/UIKit — vypuštěna konformace na
//  `WBPopUpPickerViewDelegate` (UIPickerViewDataSource/Delegate pro
//  storyboardový picker z originálního WebBLE) a její tři metody.
//  Výběr zařízení řeší `WebBLE/WBDevicePicker.swift` (naše, SwiftUI sheet)
//  přes stejný `WBPicker` protokol/`selectDeviceAt`/`cancelDeviceSearch`
//  API, beze změny zbytku souboru. Viz `WebBLE/ATTRIBUTION.md`.
